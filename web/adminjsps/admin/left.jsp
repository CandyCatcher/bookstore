...
var bar1 = new Q6MenuBar("bar1", "CANDY网络图书商城");
function load() {
	bar1.colorStyle = 2;
	bar1.config.imgDir = "<c:url value='/menu/img/'/>";
	bar1.config.radioButton=true;
	bar1.add("分类管理", "查看分类", "<c:url value='/AdminCategory?method=listAll'/>", "body");
	bar1.add("分类管理", "添加分类", "<c:url value='/adminjsps/admin/category/add.jsp'/>", "body");
	bar1.add("图书管理", "查看图书", "<c:url value='/AdminBook?method=listAll'/>", "body");
	bar1.add("图书管理", "添加图书", "<c:url value='/AdminBook?method=preAddBook'/>", "body");
